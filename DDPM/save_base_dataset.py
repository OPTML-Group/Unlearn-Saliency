...
def all_but_one_class_dataset(data_path, dataset, label_to_forget):
    ...
    if dataset == "cifar10":
        ...
        for class_idx in range(1, 10):
            class_indices = [i for i, (_, label) in enumerate(filtered_set) if label == class_idx]
        ...
        loader = torch.utils.data.DataLoader(subset_dataset, batch_size=1000, shuffle=True, drop_last=False)
