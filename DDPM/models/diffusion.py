...
class ResnetBlock(nn.Module):
    def __init__(self, *, in_channels, out_channels=None, conv_shortcut=False,
                 dropout, temb_channels=512, cemb_channels=512):
        super().__init__()
        ...
        self.conv1 = torch.nn.Conv2d(in_channels,
                                     ...
                                     stride=1,
                                     padding=1)
        self.temb_cemb_proj = torch.nn.Linear(temb_channels + cemb_channels,
                                            out_channels)
...
class Conditional_Model(nn.Module):
    ...
    def _forward_with_cond_scale(
        ...
        logits = self._forward(x, t, c, cond_drop_prob = 0.)

        if cond_scale == 0:
            return logits

        null_logits = self._forward(x, t, c, cond_drop_prob = 1.)
    ...
    def _forward(self, x, t, c, cond_drop_prob = None):
        ...
        cond_drop_prob = self.cond_drop_prob if cond_drop_prob is None else cond_drop_prob
