...
from datasets import get_dataset, data_transform, inverse_data_transform, all_but_one_class_path_dataset, get_forget_dataset
...
class Diffusion(object):
    ...
    def natural_forget(self):
        ...
        for step in range(0, self.config.training.n_iters):
            ...
            if args.method == "rl":
                random_c = torch.randint(low = 1, high = config.data.n_classes, size=forget_c.shape, device=forget_c.device)
    ...
    def train_esd(self):
        ...
        for step in range(0, self.config.training.n_iters):
            ...
            with torch.no_grad():
                ...
                e_0 = model_orig.forward(x.to(devices[1]), t.float().to(devices[1]), c.to(devices[1]), cond_drop_prob = 1., mode='train')
                e_p = model_orig.forward(x.to(devices[1]), t.float().to(devices[1]), c.to(devices[1]), cond_drop_prob = 0., mode='train')       
                
            e_n =  model.forward(x.to(devices[0]), t.float().to(devices[0]), c.to(devices[0]), cond_drop_prob = 0., mode='train')           
            e_0.requires_grad = False
            e_p.requires_grad = False
            
            loss = criteria(e_n.to(devices[0]), e_0.to(devices[0]) - (args.negative_guidance*(e_p.to(devices[0]) - e_0.to(devices[0]))))
    ...
    def sample_visualization(self, model, name, cond_scale):
        ...
        c = torch.repeat_interleave(torch.arange(config.data.n_classes), total_n_samples//config.data.n_classes).to(self.device)
    ...
    def generate_mask(self):
        ...
        args, config = self.args, self.config
        logging.info(f"Generating mask of diffusion to achieve gradient sparsity. Gamma: {config.training.gamma}, lambda: {config.training.lmbda}")
        ...
        for step in range(1):
            ...
            output = model(x, t.float(), forget_c, cond_scale=args.cond_scale, mode='test')
            ...
            for i in threshold_list:
                ...
                torch.save(hard_dict, os.path.join('results/cifar10/mask/mask_{}.pt'.format(i)))
                
            inverted_dict = {key: 1 - value for key, value in hard_dict.items()}
            torch.save(inverted_dict, os.path.join('results/cifar10/mask/inverted_mask_{}.pt'.format(i)))
                          
            random_dict = {}
            for name, param in model.named_parameters():
                if param.grad is not None: 
                    random_dict[name] = torch.randint(2, size=param.grad.shape, dtype=torch.float32)
