...
def apply_perturb(model, v, args):
    curr = 0
...
def Wfisher(data_loaders, model, criterion, args, mask=None):
    ...
    if args.imagenet_arch:
        perturb = woodfisher_im(model, retain_loader, device=device,
                            criterion=criterion, v=forget_grad-retain_grad, args=args, mask=mask)
    else:
        perturb = woodfisher(model, retain_loader, device=device,
                            criterion=criterion, v=forget_grad-retain_grad, args=args, mask=mask)       
    apply_perturb(model, args.alpha*perturb, args)
