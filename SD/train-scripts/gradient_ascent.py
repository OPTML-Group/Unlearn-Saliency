...
def gradient_ascent(class_to_forget, train_method, alpha, batch_size, epochs, lr, config_path, ckpt_path, mask_path, diffusers_config_path, device, image_size=512, ddim_steps=50):
    ...
    for epoch in range(epochs):
        with tqdm(total=len(forget_dl)) as t:
            ...
            # for i in range(1):
                ...
                # t.set_postfix(avg_loss=np.mean(losses), lr=optimizer.param_groups[0]['lr'])
                sleep(0.1)
                t.update(1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epochs', help='epochs used to train', type=int, required=False, default=10)
    ...
    gradient_ascent(classes, train_method, alpha, batch_size, epochs, lr, config_path, ckpt_path, mask_path, diffusers_config_path, device, image_size, ddim_steps)