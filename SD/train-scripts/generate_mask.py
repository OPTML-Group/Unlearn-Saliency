...
from dataset import setup_model, setup_forget_data, setup_forget_nsfw_data
...
def generate_mask(classes, c_guidance, batch_size, epochs, lr, config_path, ckpt_path, diffusers_config_path, device, image_size=512, num_timesteps=1000):
...
def generate_nsfw_mask(c_guidance, batch_size, epochs, lr, config_path, ckpt_path, diffusers_config_path, device, image_size=512, num_timesteps=1000):
    # MODEL TRAINING SETUP
    model = setup_model(config_path, ckpt_path, device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--classes', help='class corresponding to concept to erase', type=str, required=False, default='6')
    ...
    parser.add_argument('--epochs', help='epochs used to train', type=int, required=False, default=1)
    parser.add_argument('--lr', help='learning rate used to train', type=float, required=False, default=1e-5)
    parser.add_argument('--ckpt_path', help='ckpt path for stable diffusion v1-4', type=str, required=False, default='models/ldm/stable-diffusion-v1/epoch=000050.ckpt')
    ...
    parser.add_argument('--device', help='cuda devices to train on', type=str, required=False, default='4')
    ...
    parser.add_argument('--nsfw', help='class or nsfw', type=bool, required=False, default=False)
    ...
    if args.nsfw:
        print(args.nsfw)
        generate_nsfw_mask(c_guidance, batch_size, epochs, lr, config_path, ckpt_path, diffusers_config_path, device, image_size, num_timesteps)
    else:
        generate_mask(classes, c_guidance, batch_size, epochs, lr, config_path, ckpt_path, diffusers_config_path, device, image_size, num_timesteps)
