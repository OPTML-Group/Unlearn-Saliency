...
from dataset import setup_model, setup_remain_data, setup_forget_data

from einops import rearrange, repeat
from torchvision.utils import make_grid

   
def proximal_gradient(class_to_forget, train_method, alpha, batch_size, epochs, lr, config_path, ckpt_path, mask_ratio, diffusers_config_path, device, image_size=512, ddim_steps=50, second_device=None):
    ...
    gpu1_init_params = torch.concat([param.view(-1) for param in model.parameters()], dim=0)
    ...
    for epoch in range(epochs):
        ...
        # with tqdm(total=10) as time:
            ...
            # for i in range(1):
                ...
                # player -> truck
                pseudo_prompts = [descriptions[(int(class_to_forget) + 1) % 10] for label in forget_labels]
                ...
                with torch.no_grad():
                    ratio = int(mask_ratio * ((total_steps - (epoch * (len(forget_dl) + len(remain_dl)) + i + 1)) / total_steps * n_params))

                    cnt = 0
                    for param in model.parameters():
                        gpu2_params[cnt:cnt + param.numel()] = param.view(-1).to(second_device)
                        cnt += param.numel()
                    ...
                    cnt = 0
                    for param in model.parameters():
                        init_param = gpu1_init_params[cnt:cnt + param.numel()].view(param.shape)
                        param -= init_param
        ...
        for _, row in df.iterrows():
            ...
            for t in tqdm(scheduler.timesteps):
                ...
                with torch.no_grad():
                    ...
                    noise_pred = model.apply_model(latent_model_input, t_unet, text_embeddings)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--class_to_forget', help='class corresponding to concept to erase', type=str, required=True, default='0')
    ...
    parser.add_argument('--batch_size', help='batch_size used to train', type=int, required=False, default=8)
    ...
    parser.add_argument('--mask_ratio', help='mask path for stable diffusion v1-4', type=float, required=True, default=None)
    ...
    parser.add_argument('--second_device', help='cuda devices to train on', type=str, required=False, default='1')
    ...
    proximal_gradient(classes, train_method, alpha, batch_size, epochs, lr, config_path, ckpt_path, mask_ratio, diffusers_config_path, device, image_size, ddim_steps, second_device)
